...
function getStoredWishlist() {
  return JSON.parse(window.localStorage.getItem('tripsafar-favorite-destinations') || '[]')
    .map((item, index) => normalizeDestination(typeof item === 'string' ? { name: item } : item, index))
    .filter(Boolean);
}

function saveWishlist(items) {
  window.localStorage.setItem('tripsafar-favorite-destinations', JSON.stringify(items));
...
function DestinationsPage() {
  ...
  const [wishlistDestinations, setWishlistDestinations] = useState(getStoredWishlist);
  const [searchQuery, setSearchQuery] = useState(selectedDestination?.name || selectedDestination?.city || '');
  ...
  const visibleDestinations = useMemo(() => {
    const source = showWishlist ? wishlistDestinations : normalizedDestinations;
  ...
  }, [activeCategory, normalizedDestinations, searchQuery, showWishlist, wishlistDestinations]);

  const featuredDestination = visibleDestinations[0] || normalizedDestinations[0];
  const favoriteNames = new Set(wishlistDestinations.map((destination) => destination.name));
  const countryCount = new Set(normalizedDestinations.map((destination) => destination.country)).size;

  async function handleApplyLegacyFilters(event) {
    event?.preventDefault();
    await refreshDestinations({ search: searchQuery });
  }
  ...
  function handleShowWishlist() {
    setWishlistDestinations(getStoredWishlist());
  ...
  function toggleWishlist(destination) {
    setWishlistDestinations((current) => {
      const nextWishlist = current.some((item) => item.name === destination.name)
  ...
  function handleRemoveWishlist(destinationName) {
    const nextWishlist = wishlistDestinations.filter((destination) => destination.name !== destinationName);
    setWishlistDestinations(nextWishlist);
    saveWishlist(nextWishlist);
  }
    ...
    <section className="relative grid gap-6 overflow-hidden">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          { label: 'Saved Places', value: wishlistDestinations.length, icon: Heart },
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {visibleDestinations.map((destination) => {
          const isFavorite = favoriteNames.has(destination.name);
          return (
            <article key={destination._id || destination.name} className="group overflow-hidden rounded-[2rem] border border-orange-100 bg-white shadow-xl shadow-orange-100/40 transition duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-orange-100/70">
              ...
              <div className="p-5">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  ...
                  {showWishlist && (
                    <button type="button" onClick={() => handleRemoveWishlist(destination.name)} className="rounded-full border border-rose-200 px-5 py-2.5 text-sm font-black text-rose-700 transition hover:bg-rose-50">
                      Remove
                    </button>
                  )}
                </div>
