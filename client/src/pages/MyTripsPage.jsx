...
const tripTabs = ['All', 'Upcoming', 'Planned', 'Completed', 'Favorites'];
...
function MyTripsPage() {
  ...
  const filteredTrips = useMemo(() => {
    ...
    if (activeTab === 'Favorites') return trips.filter((trip) => favoriteTrips.includes(trip._id));
    return trips;
  }, [activeTab, completedTrips, favoriteTrips, trips, upcomingTrips]);
  ...
  function toggleFavoriteTrip(tripId) {
    setFavoriteTrips((current) => (current.includes(tripId) ? current.filter((id) => id !== tripId) : [...current, tripId]));
  }
      ...
      <section className="rounded-[2rem] border border-orange-100 bg-white p-5 shadow-xl shadow-orange-100/40 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <form className="grid gap-3 sm:grid-cols-[1fr_auto_auto]" onSubmit={handleApplyFilters}>
            <input name="search" value={filters.search} onChange={handleFilterChange} className="form-control" placeholder="Search trips" />
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {filteredTrips.map((trip, index) => {
            const isFavorite = favoriteTrips.includes(trip._id);
            return (
              <article key={trip._id} className="group overflow-hidden rounded-[2rem] border border-orange-100 bg-white shadow-lg shadow-orange-100/40 transition hover:-translate-y-2 hover:shadow-2xl hover:shadow-orange-100/70">
                <div className="relative h-52 overflow-hidden">
                  ...
                  <button type="button" onClick={() => toggleFavoriteTrip(trip._id)} className={`absolute right-4 top-4 grid h-10 w-10 place-items-center rounded-full ${isFavorite ? 'bg-orange-500 text-white' : 'bg-white/90 text-orange-500'}`}><Heart className={`h-5 w-5 ${isFavorite ? 'fill-current' : ''}`} /></button>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 border-t border-orange-50 pt-4">
                    ...
                    <button type="button" onClick={() => handleDuplicateTrip(trip._id)} className="text-xs font-black text-slate-500 hover:text-orange-600"><Copy className="mr-1 inline h-3.5 w-3.5" />Duplicate</button>
                    <button type="button" onClick={() => handleSaveTrip(trip)} className="text-xs font-black text-slate-500 hover:text-orange-600"><Heart className="mr-1 inline h-3.5 w-3.5" />Save Trip</button>
                    <button type="button" className="text-xs font-black text-slate-500 hover:text-orange-600"><Download className="mr-1 inline h-3.5 w-3.5" />Export PDF</button>
                    <button type="button" className="text-xs font-black text-slate-500 hover:text-orange-600"><Share2 className="mr-1 inline h-3.5 w-3.5" />Share</button>
                    {trip.status !== 'archived' && <button type="button" onClick={() => handleArchiveTrip(trip._id)} className="text-xs font-black text-slate-500 hover:text-orange-600"><Archive className="mr-1 inline h-3.5 w-3.5" />Archive</button>}
