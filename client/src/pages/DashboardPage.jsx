...
function DashboardPage() {
  ...
  const stats = [
    { label: 'Total Trips', value: summary.totalTrips || trips.length, icon: '🧳', growth: `${summary.upcomingTripsCount || 0} upcoming`, color: 'bg-[#FFFBF5] text-[#FB923C]' },
    { label: 'Saved Places', value: summary.savedPlacesCount || 0, icon: '🌍', growth: 'Supabase', color: 'bg-orange-50/70 text-[#FB923C]' },
    { label: 'Total Budget', value: currencyFormat(budget.totalBudget), icon: '💰', growth: `${summary.recentTrips?.length || 0} recent`, color: 'bg-amber-50 text-[#F59E0B]' },
    { label: 'Bookings', value: summary.bookingsCount || 0, icon: '🎟️', growth: 'Live', color: 'bg-amber-50 text-[#F59E0B]' },
  ];
    ...
    <section className="grid min-w-0 gap-5 lg:gap-6 2xl:grid-cols-[minmax(0,1fr)_340px]">
      ...
      <aside className="grid min-w-0 content-start gap-5 lg:gap-6 2xl:sticky 2xl:top-24">
        <section className="rounded-[2rem] bg-white p-5 shadow-soft ring-1 ring-black/5">
          ...
          <h2 className="mt-5 text-2xl font-black text-[#222222]">{nextTrip?.title || 'No upcoming trip'}</h2>
          <p className="mt-2 text-sm font-semibold text-[#222222]/45">{nextTrip?.startDate || 'Create a trip to see it here'}{nextTrip ? ` • ${nextTrip.travelerCount || 1} travelers` : ''}</p>
          <div className="mt-5">
