  ...
  return (
    <button type="button" onClick={onChange} className="group flex w-full items-center justify-between gap-4 rounded-2xl bg-orange-50/60 p-4 text-left ring-1 ring-orange-100 transition hover:-translate-y-0.5 hover:bg-white">
      <span className="flex items-start gap-3">
        <span className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-orange-500 ring-1 ring-orange-100"><Icon className="h-5 w-5" /></span>
        <span>
...
function ProfilePage() {
  ...
  const [settings, setSettings] = useState({ emailNotifications: true, travelReminders: true, darkMode: false });
        ...
        <aside className="grid content-start gap-6">
          ...
          <section className="rounded-[2rem] border border-orange-100 bg-white p-5 sm:p-6">
            ...
            <div className="mt-5 grid gap-3">
              <button type="button" className="flex items-center gap-3 rounded-2xl bg-orange-50/60 p-4 text-left font-black text-slate-950 ring-1 ring-orange-100 transition hover:bg-white hover:text-orange-600"><LockKeyhole className="h-5 w-5 text-orange-500" />Change password</button>
              <ToggleRow icon={Bell} label="Email notifications" description="Get trip updates and booking reminders." enabled={settings.emailNotifications} onChange={() => setSettings((current) => ({ ...current, emailNotifications: !current.emailNotifications }))} />
              <ToggleRow icon={CalendarDays} label="Travel reminders" description="Receive packing and itinerary nudges." enabled={settings.travelReminders} onChange={() => setSettings((current) => ({ ...current, travelReminders: !current.travelReminders }))} />
              <ToggleRow icon={Moon} label="Dark mode" description="Preview a darker dashboard experience." enabled={settings.darkMode} onChange={() => setSettings((current) => ({ ...current, darkMode: !current.darkMode }))} />
              <button type="button" className="flex items-center gap-3 rounded-2xl bg-orange-50/60 p-4 text-left font-black text-slate-950 ring-1 ring-orange-100 transition hover:bg-white hover:text-orange-600"><ShieldCheck className="h-5 w-5 text-orange-500" />Privacy settings</button>
            </div>
          </section>
              ...
              {dummyActivities.map((activity) => {
                ...
                return (
                  <article key={activity.title} className="flex gap-3 rounded-2xl bg-orange-50/50 p-4 ring-1 ring-orange-100 transition hover:-translate-y-0.5 hover:bg-white">
                    <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${activity.tone}`}><Icon className="h-5 w-5" /></span>
                    <div>
