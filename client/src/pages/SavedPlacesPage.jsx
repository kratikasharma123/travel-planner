...
const collectionTemplates = [
  { title: 'Honeymoon Ideas', matcher: (place) => includesAny(place, ['honeymoon', 'romantic', 'luxury', 'beach']), icon: Heart, tone: 'bg-rose-50 text-rose-600' },
  { title: 'Budget Trips', matcher: (place) => includesAny(place, ['budget', 'budget friendly', 'low-cost']) || String(place.budgetLevel || '').toLowerCase().includes('budget'), icon: Wallet, tone: 'bg-emerald-50 text-emerald-700' },
  { title: 'Weekend Escapes', matcher: (place) => includesAny(place, ['weekend', 'city', 'short trip', 'escape']), icon: Plane, tone: 'bg-orange-50 text-orange-600' },
  { title: 'Food Places', matcher: (place) => includesAny(place, ['food', 'restaurant', 'restaurants', 'cafe', 'cafes']), icon: Utensils, tone: 'bg-amber-50 text-amber-700' },
  { title: 'Adventure Spots', matcher: (place) => includesAny(place, ['adventure', 'mountain', 'mountains', 'activity', 'activities', 'hidden gem', 'hidden gems']), icon: Mountain, tone: 'bg-teal-50 text-teal-700' },
];

const aiSuggestions = [
  { title: 'Similar places you may like', description: 'AI feature will be added in a future milestone.', icon: Sparkles, tone: 'bg-orange-50 text-orange-600' },
  { title: 'Best time to visit', description: 'AI feature will be added in a future milestone.', icon: CalendarDays, tone: 'bg-emerald-50 text-emerald-700' },
  { title: 'Build itinerary from saved places', description: 'AI feature will be added in a future milestone.', icon: Route, tone: 'bg-amber-50 text-amber-700' },
  { title: 'Compare saved destinations', description: 'AI feature will be added in a future milestone.', icon: Compass, tone: 'bg-teal-50 text-teal-700' },
];

function getPlaceText(place) {
  return [place.category, place.name, place.city, place.country, place.notes, place.budgetLevel, ...(place.tags || [])].join(' ').toLowerCase();
}

function includesAny(place, keywords = []) {
...
function matchesCategory(place, category) {
  ...
  const haystack = [place.category, place.name, place.city, place.country, place.notes, ...(place.tags || [])].join(' ').toLowerCase();
  ...
  return (
    <article className="group overflow-hidden rounded-[2rem] border border-orange-100 bg-white shadow-xl shadow-orange-100/40 transition duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-orange-100/70">
      <div className="relative h-64 overflow-hidden">
        <img src={place.imageUrl || 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=900&q=90'} alt={place.name} className="h-full w-full object-cover transition duration-700 group-hover:scale-110" loading="lazy" />
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 flex items-center gap-1 text-sm font-semibold text-slate-500"><MapPin className="h-4 w-4 text-orange-500" />{place.city || place.name}, {place.country}</p>
          </div>
          <span className="inline-flex items-center gap-1 rounded-full bg-white px-3 py-1 text-xs font-black text-slate-800 ring-1 ring-orange-100"><Star className="h-3.5 w-3.5 fill-orange-400 text-orange-400" />{place.rating || 4.7}</span>
        </div>

        <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-2xl bg-orange-50 p-3"><Wallet className="h-4 w-4 text-orange-500" /><p className="mt-2 font-black capitalize text-slate-800">{place.budgetLevel || 'Mid-range'}</p></div>
          <div className="rounded-2xl bg-emerald-50 p-3"><CalendarDays className="h-4 w-4 text-emerald-600" /><p className="mt-2 font-black text-slate-800">{place.bestTime || 'Anytime'}</p></div>
        </div>

        <p className="mt-4 line-clamp-2 rounded-2xl bg-amber-50/70 p-3 text-sm leading-6 text-slate-600">{place.notes || 'Saved for future planning.'}</p>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <button type="button" onClick={() => onRemove(place._id)} className="rounded-full border border-rose-200 px-4 py-2 text-sm font-black text-rose-700 transition hover:bg-rose-50"><Trash2 className="mr-1 inline h-4 w-4" />Remove</button>
        </div>
      </div>
    </article>
  );
...
function SavedPlacesPage() {
  ...
  const { savedPlaces, isLoading, error, removeSavedPlace } = useSavedPlaces();
  ...
  const collections = useMemo(() => collectionTemplates.map((collection) => {
    const places = savedPlaces.filter(collection.matcher);
    return {
      ...collection,
      count: places.length,
      preview: places.slice(0, 2).map((place) => place.name).join(', '),
    };
  }), [savedPlaces]);
  ...
  function handlePlanTrip(place) {
    navigate('/planner', { state: { destination: { id: place.destinationId, name: place.name, country: place.country, city: place.city, imageUrl: place.imageUrl, tag: place.category } } });
  }
  ...
  return (
    <section className="relative grid gap-6 overflow-hidden">
      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-white/80 p-6 shadow-2xl shadow-orange-100 sm:p-8 lg:p-10">
        <img src="https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1800&q=90" alt="Saved places beach travel background" className="absolute inset-0 h-full w-full scale-105 object-cover blur-[1.5px]" />
      ...
      {(error || success || isLoading) && (
        <div className="grid gap-3">
          {isLoading && <p className="rounded-2xl bg-white p-4 text-sm font-bold text-slate-600 shadow-soft">Loading saved places...</p>}
      ...
      {filteredPlaces.length === 0 && !isLoading ? (
        ...
        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {filteredPlaces.map((place) => <SavedPlaceCard key={place._id} place={place} onRemove={handleRemove} onPlanTrip={handlePlanTrip} />)}
        </section>
      )}
          ...
          {collections.map((collection) => {
            ...
            return (
              <article key={collection.title} className="rounded-[1.5rem] border border-orange-100 bg-gradient-to-br from-white to-orange-50 p-4 transition hover:-translate-y-1 hover:shadow-lg hover:shadow-orange-100">
                ...
                <p className="mt-1 text-sm font-semibold text-slate-500">{collection.count} saved {collection.count === 1 ? 'place' : 'places'}</p>
                <p className="mt-2 text-xs font-semibold leading-5 text-slate-400">{collection.preview || 'No matching saved places yet'}</p>
              </article>
            );
          })}
        </div>
